...
export const extraInfoToRegiserGoole_Get=async(req:any,res:any)=>{
    
        console.log("====1like111",req.params.id)
       var conflic= await  User.find({ google_id:req.params.id })
        console.log("email already exits",conflic);
        if(conflic[0].DOB===undefined){
            res.redirect(`${CLIENT_URL}google/moreinfo/${req.params.id }`);
        }else{
            res.redirect(`${CLIENT_URL}dashboard`);
        }
        
    }
    export const extraInfoToRegiserGoogle_Post=async(req:any,res:any)=>{
